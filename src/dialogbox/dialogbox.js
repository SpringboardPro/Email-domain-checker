...
function createEmailCheckBoxList (arg) {
  ...
  if (messageType === 'appointment') {
    ...
    document.getElementById('warning').innerHTML = 'Cannot send email. The selected email addresses are incorrect.'
  }
...
function createDecoyEmail (unstringifiedEmails) {
  ...
  let name = emails[i].emailAddress.slice(0, emails[i].emailAddress.indexOf('@'))
